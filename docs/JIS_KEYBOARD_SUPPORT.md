...
**直接 HID キーコードを使用しているキー:**
- `LG(INT3)` — バックスラッシュ（`\`）入力用。後述の「INT1 / INT3 の互換性」を参照
...
ZMK ドキュメントでは `INT1` (`INT_RO`) と `INT3` (`INT_YEN`) の macOS 互換性が「❔」（未テスト）となっている。

### 実機での検証結果（macOS + JIS 設定）

| HID キーコード | ZMK ドキュメント上の期待値 | 実際の出力 |
|---------------|-------------------------|-----------|
| INT1 | `\` | `_` |
| LS(INT1) | `_` | `_`（JIS_UNDER として使用中） |
| INT3 | `¥` | `¥` |
| LS(INT3) | `\|` | `\|`（JIS_PIPE として使用中） |
| LG(INT3) | - | `\`（バックスラッシュ） |

**重要**: `INT1` は ZMK ドキュメント上 `\` を出力するとされているが、macOS JIS 環境では `_` が出力される。バックスラッシュを入力するには `LG(INT3)`（Cmd+¥）を使用する必要がある。

この知見に基づき、Layer 2 では以下のように割り当てている:
- バックスラッシュ（`\`）: `&kp LG(INT3)`
- アンダースコア（`_`）: `&kp JIS_UNDER`（= `LS(INT1)`）
- パイプ（`|`）: `&kp JIS_PIPE`（= `LS(INT3)`）
